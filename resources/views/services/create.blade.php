...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
    ...
    <form action="{{ route('services.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="w-full px-3 py-2 border border-gray-300 rounded" rows="3">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="skill_type" class="w-full px-3 py-2 border border-gray-300 rounded" required>
                ...
                @foreach($skillTypes as $skillType)
                    <option value="{{ $skillType }}" {{ old('skill_type') == $skillType ? 'selected' : '' }}>
                        {{ $skillType }}
                    </option>
                @endforeach
            </select>

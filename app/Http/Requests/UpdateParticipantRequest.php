...
class UpdateParticipantRequest extends FormRequest
{
    ...
    {
        $participantId = $this->route('participant') ? $this->route('participant')->participant_id : $this->route('id');
        ...
        return [
            ...
            'email' => [
                ...
                'email',
                'max:255',
                function ($attribute, $value, $fail) use ($participantId) {
                    $exists = Participant::whereRaw('LOWER(email) = LOWER(?)', [strtolower($value)])

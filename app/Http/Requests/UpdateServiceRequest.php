...
class UpdateServiceRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Scoped Uniqueness Rule - Service name must be unique within a Facility (excluding current record)
            $facilityId = $this->input('facility_id');
            $name = $this->input('name');
            $serviceId = $this->route('service') ? $this->route('service')->service_id : $this->route('id');
            
            if ($facilityId && $name && $serviceId) {

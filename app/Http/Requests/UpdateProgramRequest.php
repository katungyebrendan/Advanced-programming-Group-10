...
class UpdateProgramRequest extends FormRequest
{
    ...
    {
        $programId = $this->route('program') ? $this->route('program')->program_id : $this->route('id');
        
        return [
            // Required Fields Rule & Uniqueness Rule (case-insensitive, excluding current record)
            'name' => [
                'required',
                'string',
                'max:255',
                function ($attribute, $value, $fail) use ($programId) {
                    $exists = Program::whereRaw('LOWER(name) = LOWER(?)', [strtolower($value)])
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Lifecycle Protection Rule - Check if program can be deleted/modified
            $programId = $this->route('program') ? $this->route('program')->program_id : $this->route('id');
            if ($programId) {
                $program = Program::find($programId);
                if ($program && !$program->canBeDeleted()) {
                    // Add warning but don't block update
                    $validator->after(function ($validator) {
                        $validator->getMessageBag()->add('warning', 'Program has Projects; some changes may affect related data.');
                    });
                }
            }
        });
